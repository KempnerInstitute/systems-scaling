...
class RotaryEmbedding(nn.Module):
    ...
    def __init__(self, config: ModelConfig, cache: BufferCache):
        ...
        self.__cache = cache
        # Warm up cache.
        self.get_rotary_embedding(config.context_length, _non_meta_init_device(config))
...
class OLMoBlock(nn.Module):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        # )
        self.attn_out = LinearDebiased(
            config.d_model, config.d_model, bias=config.include_bias
        )
...
class OLMo(nn.Module):
    def __init__(self, config: ModelConfig, init_params: bool = True):
        ...
        if not (self.config.alibi or self.config.rope):
            self.transformer.update(
                {"wpe": nn.Embedding(config.context_length, config.d_model, device=config.init_device)}
            )
        ...
        if self.config.alibi:
            get_causal_attention_bias(self.__cache, config.context_length, _non_meta_init_device(config))
            self.get_alibi_attention_bias(config.context_length, _non_meta_init_device(config))
    ...
    def num_fwd_flops(self):
        ...
        params_flops_per_seq = params_flops_per_token * self.config.context_length
        # there are 2 FLOPS per mac; there is A=Q*K^T and out=A*V ops (ie mult by 2)
        attn_flops_per_seq = (
            self.config.n_layers * 2 * 2 * (self.config.d_model * (self.config.context_length**2))
        )
...
class HFMixinOLMo(OLMo, PyTorchModelHubMixin):
    def __init__(self, config: Dict):
        config = ModelConfig(**config)
        super().__init__(config, init_params=True)